...
int main(int argc, char ** argv) {
	...
	if (gOptUsingRootGeom) {
		...
		// creating & configuring a root geometry driver - using the creator that requires a TGeoManager pointer
		TGeoManager * gm = TGeoManager::Import(gOptRootGeom.c_str());
		geometry::ROOTGeomAnalyzer * rgeom = new geometry::ROOTGeomAnalyzer(gm);
	...
	// *************************************************************************
	GFluxI * flux_driver = genie::flux::GFluxDriverFactory::Instance().GetFluxDriver(gOptFluxDriver);
	if (!flux_driver) {
		// couldn't get the requested flux driver ?
		std::ostringstream s;
		s << "Known FluxDrivers:\n";
		const std::vector<std::string>& known = genie::flux::GFluxDriverFactory::Instance().AvailableFluxDrivers();
	...
	if (!gOptUsingHistFlux) {
		genie::flux::GFluxFileConfigI* flux_file_config = dynamic_cast<genie::flux::GFluxFileConfigI*>(flux_driver);
		if (!flux_file_config) {
			LOG("gevgen_bdx", pFATAL) << "Failed to get GFluxFileConfigI driver from GFluxDriverFactory\n" << "when using \"" << gOptFluxDriver << "\"";
			exit(1);
		}
		...
		if (gOptFluxPdg.size() > 0) {
			...
			for (; itr != gOptFluxPdg.end(); ++itr)
				s << (*itr) << " ";
			LOG("gevgen_bdx", pNOTICE) << "Limiting to nu PDGs: " << s.str();
		}
	} else {
		//
		...
		//Using simple geoemtry
			...
			if (!hist_flux_driver) {
				LOG("gevgen_bdx", pFATAL) << "Failed to get GCylinderTH1Flux driver from GFluxDriverFactory\n" << "when using " << gOptFluxDriver;
				exit(1);
	...
	if (fluxFileConfigI) {
		fluxFileConfigI->GetBranchInfo(branchNames, branchClassNames, branchObjPointers);
...
void LoadExtraOptions(void) {
	...
	for (size_t ilib = 0; ilib < extraLibs.size(); ++ilib) {
		...
		if (loadStatus == 0) {
			statWords = "successfully loaded";
		} else if (loadStatus == 1) {
			statWords = "already had loaded";
		} else if (loadStatus == -1) {
			statWords = "couldn't find library";
...
void GetCommandLineArgs(int argc, char ** argv) {
	...
	if (parser.OptionExists('e')) {
		...
		if (nue.find(",") != string::npos) {
			...
			LOG("gevgen_bdx", pERROR) << "Energy must be speficified as Emin,Emax always. If ignoring Emin: use ,Emax with empty string before comma. If ignoring Emax: use ,Emax with empty string after command";
			PrintSyntax();
			exit(1);
		}
	}	//-e
	...
	if (gOptUsingRootGeom) {
		gminfo << "Using ROOT geometry - file = " << gOptRootGeom << ", top volume = " << ((gOptRootGeomTopVol.size() == 0) ? "<master volume>" : gOptRootGeomTopVol) << ", max{PL} file = " << ((gOptExtMaxPlXml.size() == 0) ? "<none>" : gOptExtMaxPlXml) << ", length  units  = " << lunits << ", density units  = " << dunits;
	} else {
...
void CreateRockBoxSelection(string fidcut, GeomAnalyzerI* geom_driver) {
	...
	if (!rockonly)
		rocksel->MakeSphere(0, 0, 0, 1.0e-10);
	else
		rocksel->MakeBox(xyzmin, xyzmax);
...
void ParseFluxHst3D(string flux) {
	...
	if (fluxv.size() < 2) {
		LOG("gevgen_bdx", pFATAL) << "You need to specify both a flux file and 1 histogram for one species";
		PrintSyntax();
		exit(1);
	}
	...
	if (!accessible_flux_file) {
		LOG("gevgen_bdxl", pFATAL) << "Can not access flux file: " << gOptFluxFile;
		PrintSyntax();
		exit(1);
	}
	...
	if (fluxv[1] == "DEFAULT") {
		...
		for (unsigned int inu = 2; inu < fluxv.size(); inu++) {
			...
			// create a local copy of the input histograms
			TH1D * spectrum = (TH1D*) ihst->Clone();
			spectrum->SetName(histo.c_str());
		...
		for (unsigned int inu = 1; inu < fluxv.size(); inu++) {
			...
			if (open_bracket == string::npos || close_bracket == string::npos) {
				LOG("gevgen_bdx", pFATAL) << "You made an error in specifying the flux histograms";
				PrintSyntax();
				exit(1);
			}
			string::size_type ibeg = 0;
