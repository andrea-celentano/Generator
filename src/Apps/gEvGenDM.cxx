...
void GenerateEventsAtFixedInitState(void) {
	...
	if (!unitary) {
		LOG("gevgen_dm", pFATAL) << "Cross-section risks exceeding unitarity limit - Exiting";
		exit(1);
	}*/
	...
	while (ievent < gOptNevents) {
		...
		if (!event) {
			LOG("gevgen_dm", pNOTICE) << "Last attempt failed. Re-trying....";
			continue;
		}
...
//
	...
	if (gOptDMEnergyRange < 0)
		flux_driver = MonoEnergeticFluxDriver();
	else
...
//
//
	flux::GMonoEnergeticFlux *flux = new flux::GMonoEnergeticFlux(gOptDMEnergy, kPdgDarkMatter);
...
//
	...
	bool input_is_root_file = gOptFlux.find(".root") != string::npos && gOptFlux.find(",") != string::npos;
	if (input_is_text_file) {
		...
		for (int ibin = 1; ibin <= hst->GetNbinsX(); ibin++) {
			if (hst->GetBinLowEdge(ibin) + hst->GetBinWidth(ibin) > emax || hst->GetBinLowEdge(ibin) < emin) {
...
void GetCommandLineArgs(int argc, char **argv) {
	...
	if (parser.OptionExists('n')) {
		...
		LOG("gevgen_dm", pINFO) << "Unspecified number of events to generate - Using default";
	...
	if (parser.OptionExists('o')) {
		...
		if (gOptOutFileName.find_last_of(".") != string::npos) gOptStatFileName = gOptStatFileName.substr(0, gOptOutFileName.find_last_of("."));
	...
	if (parser.OptionExists('s')) {
		LOG("gevgen_dm", pWARN) << "-s option no longer available. Please read the revised code documentation";
		gAbortingInErr = true;
		exit(1);
	}
	...
	if (parser.OptionExists('e')) {
		...
		if (dme.find(",") != string::npos) {
			...
			if (!using_flux) {
				LOG("gevgen_dm", pWARN) << "No flux was specified but an energy range was input!";
				LOG("gevgen_dm", pWARN) << "Events will be generated at fixed E = " << gOptDMEnergy << " GeV";
				gOptDMEnergyRange = -1;
			}
		} else {
	...
	if (parser.OptionExists('t')) {
		...
		if (tgtmix.size() == 1) {
			...
			for (; tgtmix_iter != tgtmix.end(); ++tgtmix_iter) {
				...
				LOG("Main", pNOTICE) << "Adding to target mix: pdg = " << pdg << ", wgt = " << wgt;
				gOptTgtMix.insert(map<int, double>::value_type(pdg, wgt));
			}    //tgtmix_iter
		}    //>1
	...
	LOG("gevgen_dm", pNOTICE) << "MC Run Number: " << gOptRunNu;
	if (gOptRanSeed != -1) {
		LOG("gevgen_dm", pNOTICE) << "Random number seed: " << gOptRanSeed;
	} else {
		LOG("gevgen_dm", pNOTICE) << "Random number seed was not set, using default";
	}
	LOG("gevgen_dm", pNOTICE) << "Number of events requested: " << gOptNevents;
	if (gOptInpXSecFile.size() > 0) {
		LOG("gevgen_dm", pNOTICE) << "Using cross-section splines read from: " << gOptInpXSecFile;
	} else {
		LOG("gevgen_dm", pNOTICE) << "No input cross-section spline file";
	}
	LOG("gevgen_dm", pNOTICE) << "Flux: " << gOptFlux;
	LOG("gevgen_dm", pNOTICE) << "Generate weighted events? " << gOptWeighted;
	if (gOptDMEnergyRange > 0) {
		LOG("gevgen_dm", pNOTICE) << "Dark matter energy: [" << gOptDMEnergy << ", " << gOptDMEnergy + gOptDMEnergyRange << "]";
	} else {
		LOG("gevgen_dm", pNOTICE) << "Dark matter energy: " << gOptDMEnergy;
	}
	LOG("gevgen_dm", pNOTICE) << "Dark matter mass: " << gOptDMMass;
	LOG("gevgen_dm", pNOTICE) << "Target code (PDG) & weight fraction (in case of multiple targets): ";
	LOG("gevgen_dm", pNOTICE) << "Mediator mass ratio: " << gOptMedRatio;
	map<int, double>::const_iterator iter;
	for (iter = gOptTgtMix.begin(); iter != gOptTgtMix.end(); ++iter) {
		int tgtpdgc = iter->first;
		double wgt = iter->second;
		LOG("gevgen_dm", pNOTICE) << " >> " << tgtpdgc << " (weight fraction = " << wgt << ")";
	}
	LOG("gevgen_dm", pNOTICE) << "\n";
...
bool CheckUnitarityLimit(void) {
	...
	if (!unitary) {
		LOG("gevgen_dm", pWARN) << "Estimated a cross-section " << xsec_est / cm2 << " cm^2";
		LOG("gevgen_dm", pWARN) << "Unitarity limit set to " << xsec_lim / cm2 << " cm^2";
	}
	return unitary;
}
//____________________________________________________________________________
